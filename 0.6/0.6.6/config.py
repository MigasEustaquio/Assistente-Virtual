...
class Thread_class(threading.Thread):
	...
	def set_tempo(self, tempo):
		self.tempo = tempo #receber em minutos
	...
	def run(self):
		...
		minuto = int(datetime.datetime.now().strftime("%M"))

		tempo_atual = (hora*60)+minuto

		tempo_para_retornar = tempo_atual + 2

		self.tempo = 0

		while tempo_atual < tempo_para_retornar:
			...
			if self.tempo != 0:
				tempo_para_retornar = tempo_atual + self.tempo
				self.tempo = 0
